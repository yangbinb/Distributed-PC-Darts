...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3):
    ...
    self.arch_alphas_normal = nn.Parameter(1e-3*torch.randn(k, num_ops))
    self.arch_alphas_reduce = nn.Parameter(1e-3*torch.randn(k, num_ops))
    self.arch_betas_normal = nn.Parameter(1e-3*torch.randn(k))
    self.arch_betas_reduce = nn.Parameter(1e-3*torch.randn(k))
    self.register_parameter('arch_alphas_normal',self.arch_alphas_normal)
    self.register_parameter('arch_alphas_reduce',self.arch_alphas_reduce)
    self.register_parameter('arch_betas_normal',self.arch_betas_normal)
    self.register_parameter('arch_betas_reduce',self.arch_betas_reduce)
    self.alphas_normal = self.arch_alphas_normal
    self.alphas_reduce = self.arch_alphas_reduce
    self.betas_normal = self.arch_betas_normal
    self.betas_reduce = self.arch_betas_reduce
